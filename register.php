...
<body>
  ...
  <form action="" method="POST">
    ...
    <select name="set" id="">
      ...
      <?php
      $display_sets = "SELECT * FROM sets";
      $query_sets = mysqli_query($con, $display_sets);
  ...
  if(isset($_POST['register'])){
    ...
    && empty($_POST['password'])){
      ...
      if(mysqli_num_rows($query_student) > 0){
        ...
        if($query_insert){
          $insert_course = "INSERT INTO users_course (`user_id`, `course_id`, `date_time_created`)
          VALUES ('$id_number', '$course', '$added_on')";
          $query_insert_course = mysqli_query($con, $insert_course);
          if($query_insert_course){
            $insert_set = "INSERT INTO users_set (`user_id`, `set_id`, `date_time_created`)
            VALUE ('$id_number', '$set', '$added_on')";
            $query_insert_set = mysqli_query($con, $insert_set);
            if($query_insert_set){
              echo '<script>swal({
            ...
            echo '<script>swal({
              title: "Error!",
              text: "Something went wrong in users_set!",
              icon: "warning",
              }).then(function() {
              // Redirect the user
              window.location.href="register.php";
            ...
            echo '<script>swal({
              title: "Error!",
              text: "Something went wrong in users_course!",
              icon: "warning",
              }).then(function() {
              // Redirect the user
              window.location.href="register.php";
